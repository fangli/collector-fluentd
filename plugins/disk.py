...
# @@Create Date: 2013-07-09 17:32:01
# @@Modify Date: 2013-12-06 11:46:31
# @@Function:
# *********************************************************#
...
def main():
    ...
    for line in f_mounts:
        ...
        # fs_passno   # Order in which filesystem checks are done at reboot
        # time
        try:
        ...
        if fs_vfstype in FSTYPE_IGNORE or fs_vfstype.startswith("fuse."):
            continue
        if (
            fs_file.startswith("/dev")
            or fs_file.startswith("/sys")
            or fs_file.startswith("/proc")
            or fs_file.startswith("/lib")
            or fs_file.startswith("/selinux")
        ):
            continue
